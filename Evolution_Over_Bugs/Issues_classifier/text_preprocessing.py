...
def remove_name(input_text_or_list: Union[str, List[str]]) -> List[str]:
    ...
    if isinstance(input_text_or_list, str):
        ...
        processed_tokens = [token for token in tokens
                            if (not name_searcher.search(token)) and
                               (not name_searcher.search(token))]
    else:
        processed_tokens = [token for token in input_text_or_list
                            if (not name_searcher.search(token)) and
                               (not name_searcher.search(token)) and token is not None and len(token) > 0]
    return processed_tokens
...
if __name__ == '__main__':
    text_to_process = 'This is Mohamed, man. My email is mohamedayman@15069@gmail.com. COntact me whenever u re avaialble'
