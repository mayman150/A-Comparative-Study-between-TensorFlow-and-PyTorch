...
def __main__():
    ...
    parser.add_argument("--data_dir", help="directory to store the data", default="output/")
    ...
    if args.pytorch:
        ...
        write_issues_to_csv(g, issues, os.path.join(args.data_dir, 'PyTorch_' + args.file_suffix), ['Issue Number', 'Issue Title', 'Issue Body', 'Time created', 'Time closed', 'Number of Assignees', 'Number of Comments', 'Tags'])

    #Tensorflow
    if args.tensorflow:
        issues = get_issues(g, 'tensorflow/tensorflow', state=args.state)
        # write issues to a CSV file
        write_issues_to_csv(g, issues,  os.path.join(args.data_dir, 'Tensorflow_' + args.file_suffix), ['Issue Number', 'Issue Title', 'Issue Body', 'Time created','Time closed' ,  'Number of Assignees', 'Number of Comments', 'Tags'])
